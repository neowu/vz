...
use objc2_foundation::NSObjectNSKeyValueObserverRegistration as _;
    ...
    impl VZMacOSInstallerObserver {
        ...
        fn observe_value(
            ...
            if let Some(change) = change {
                ...
                info!("install progress: {:.2}%", percent.floatValue() * 100.0);
            }
        }
    }
);
...
impl VZMacOSInstallerObserver {
    fn new(progress: Retained<NSProgress>) -> Retained<Self> {
        ...
        let observer_progress = &observer.ivars().progress;
        unsafe {
            observer_progress.addObserver_forKeyPath_options_context(
                &observer,
