    ...
    T: Serialize + fmt::Debug,
{
    serde_json::to_string_pretty(object)
        .unwrap_or_else(|err| panic!("failed to serialize, object={object:?}, err={err}"))
}
...
where
    T: Serialize + fmt::Debug,
{
    let value = serde_json::to_string(enum_value)
        .unwrap_or_else(|err| panic!("failed to serialize, enum={enum_value:?}, err={err}"));
    value.strip_prefix('"').and_then(|value| value.strip_suffix('"')).map(str::to_owned).unwrap_or(value)
}
